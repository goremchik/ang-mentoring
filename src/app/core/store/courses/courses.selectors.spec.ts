...
// Mocks
import { initialStore, readyStore, courses, authors } from 'src/app/mock';
...
describe('Courses selectors', () => {
  ...
  it('getAuthors', () => {
    expect(selectors.getAuthors(initialStore)).toEqual([]);
    expect(selectors.getAuthors(readyStore)).toEqual(authors);
  });
});
